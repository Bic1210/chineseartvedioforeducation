...
def analyze_prompt_deepseek(user_input):
    api_key = os.getenv("DEEPSEEK_API_KEY")
    if not api_key or api_key.startswith("sk-...") or api_key == "...":
        print("DEBUG: DeepSeek 使用 Mock 模式")
        return [
    ...
    try:
        ...
        print(f"DEBUG: 正在请求 DeepSeek 优化: {user_input[:20]}...")
        ...
        print(f"DEBUG: DeepSeek 返回 {len(prompts)} 个提示词")
        return prompts
    except Exception as e:
        print(f"DEBUG: DeepSeek 接口调用异常: {str(e)}")
